import { MindfullField, MindfullItem, MindfullTag } from "./types"
...
export function addTag(item: MindfullItem, name: string, origin?: {server: string, user: string}): MindfullTag {
    const tag = {
        name,
        origin
    }
...
export function getFieldDate(field: MindfullField): Date {
    if(field.date && field.time)    return new Date(`${field.date}, ${field.time}`)
